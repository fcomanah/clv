...
<form name="bcash" action="https://www.bcash.com.br/checkout/pay/" method="post">
    ...
    <input name="url_retorno" type="hidden" value="<?php=$_SERVER['SERVER_NAME']?>/clv/retorno.php">
    <input name="url_aviso" type="hidden" value="<?php=$_SERVER['SERVER_NAME']?>/clv/aviso.php">
    <input name="id_pedido" type="hidden" value="<?php=$_SESSION['chave']?>">
    ...
    ?>
    <input name="frete" type="hidden" value="<?php=get_frete()?>">

    <input name="email" type="hidden" value="<?php=$_SESSION['email']?>" >
    <input name="cpf" type="hidden" value="<?php=$_SESSION['cpf']?>">
    <input name="nome" type="hidden" value="<?php=$_SESSION['name']?>">
    <input name="telefone" type="hidden" value="<?php=$_SESSION['tel']?>">
    <input name="cep" type="hidden" value="<?php=$_SESSION['sCepDestino']?>">

	<div align="center">
        <h1 align="center">Pagamento</h1>
        <p><a href="javascript:clear();" title="Clique aqui se não for você!"><?php=$_SESSION['name']?></a>, o total da compra é </br><h3><?php='R$ '.number_format(get_order_total(), 2, '.', ' ')?></h3></p>
...
<?php}else if (is_array($_SESSION['cart'])){?>

<form name="form1" action="pagamento.php" method="POST">
    <input type="hidden" name="command" value="verify" />
	<div align="center">
        <h1 align="center">Seus Dados</h1>
        <div style="color:<?php=$msg_color?>"><?php=$msg?></div>
        <table border="0" cellpadding="2px">
            <tr><td>Nome:</td><td><input type="text" name="name" required value="<?php=$_SESSION['name']?>"/></td></tr>
            <tr><td>Sobrenome:</td><td><input type="text" name="sname" required value="<?php=$_SESSION['sname']?>"/></td></tr>
            <tr><td>Email:</td><td><input type="email" name="email" required value="<?php=$_SESSION['email']?>"/></td></tr>
